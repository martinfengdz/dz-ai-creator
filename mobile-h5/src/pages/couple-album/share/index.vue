...
const completedPages = computed(() => pages.value.filter((page) => page.status === 'succeeded' && pagePreview(page)).length)
const coverPage = computed(() =>
  pages.value.find((page) => page.id === album.value?.cover_page_id && pagePreview(page)) ||
  pages.value.find((page) => pagePreview(page)) ||
  null
)
...
const shareQuery = computed(() => {
  const token = encodeShareValue(share_token.value)
  return token ? `token=${token}` : ''
})
const sharePath = computed(() => {
  const query = shareQuery.value
  return `${routes.coupleAlbumShare}${query ? `?${query}` : ''}`
})
...
onLoad((options = {}) => {
  share_token.value = `${options.token || options.share_token || ''}`.trim()
...
async function ensureDownloadAuth() {
  const user = await requireAuth({ redirect: sharePath.value })
  return Boolean(user)
}
...
async function loadSharedAlbum() {
  ...
  errorMessage.value = ''
  try {
    const payload = await api.getPublicCoupleAlbum(share_token.value)
...
<template>
  <view class="safe-page album-share-page">
    ...
    <view v-if="loading" class="state-strip">相册读取中...</view>
    ...
    <template v-if="album">
      <view class="cover-panel">
        ...
        <view class="cover-copy">
          ...
          <text>{{ location }} · {{ statusText(album.status) }}</text>
        </view>
      </view>
...
.topbar-copy text:first-child {
  color: $ink;
  font-size: 28rpx;
  font-weight: 950;
}

.topbar-copy text:last-child {
  color: $muted;
  font-size: 21rpx;
  font-weight: 820;
}
