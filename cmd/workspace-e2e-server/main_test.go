...
func TestCommerceFoundationSeed(t *testing.T) {
	...
	if err := db.AutoMigrate(&core.User{}, &core.UserRole{}, &core.CreditBalance{}, &core.CreditTransaction{}, &core.GenerationRecord{}, &core.Work{}, &core.ReferenceAsset{}); err != nil {
		t.Fatal(err)
	}
	if err := ecommerce.MigrateSQLiteFoundationSchema(t.Context(), db); err != nil {
		t.Fatal(err)
	}
	role := core.UserRole{Code: "standard_user", Name: "标准用户"}
