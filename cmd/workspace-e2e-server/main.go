...
func (p *stubImageProvider) Generate(ctx context.Context, input core.ImageGenerationInput) (core.ImageGenerationResult, *core.ProviderError) {
	p.mu.Lock()
	defer p.mu.Unlock()
	...
	if strings.Contains(input.Prompt, "章节=selling_points") && !p.sellingPointsFailedBySource[source] {
		...
		return core.ImageGenerationResult{}, &core.ProviderError{Code: "provider_policy_rejected", Message: "E2E 预期的单项生成失败"}
	}
	return core.ImageGenerationResult{
...
func main() {
	...
	db, err := gorm.Open(sqlite.Open(filepath.Join(root, "core.db")), &gorm.Config{})
	if err != nil {
	...
	application, err := core.NewWithDependencies(cfg, db, &stubImageProvider{})
	if err != nil {
...
func seedWorkspaceE2E(db *gorm.DB, assetRoot string) error {
	...
	if err := db.Create(&core.CreditBalance{UserID: user.ID, AvailableCredits: 100}).Error; err != nil {
		return fmt.Errorf("create balance: %w", err)
	}
	if err := db.Create(&core.CreditTransaction{
		UserID:       user.ID,
	...
	work := core.Work{
		...
		Model:              "gpt-image-2",
		Status:             core.GenerationStatusSucceeded,
		Visibility:         core.WorkVisibilityPrivate,
		AssetKey:           "e2e/history.png",
		MIMEType:           "image/png",
...
func seedCommerceFoundationE2E(db *gorm.DB, userID, workID uint) error {
	...
	var reference core.ReferenceAsset
