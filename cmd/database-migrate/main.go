...
import (
	...
	"dz-ai-creator/internal/service/generation"
...
func run(args []string, getenv envLookup, output io.Writer) int {
	...
	if *scope == "image-generation" {
		switch *action {
		case "up":
			var report generation.ImageGenerationQueueMigrationReport
			report, err = generation.MigrateImageGenerationQueueSchema(ctx, db)
			if err == nil {
		...
		case "status":
			...
			jobs, leases, err = generation.ImageGenerationQueueMigrationStatus(ctx, db)
			if err == nil {
