...
func TestOptionAssetManifestContainsThirteenExistingSources(t *testing.T) {
	...
	if counts[core.CoupleAlbumOptionTypeLocation] != 5 {
		t.Fatalf("location asset count = %d, want 5", counts[core.CoupleAlbumOptionTypeLocation])
	}
	if counts[core.CoupleAlbumOptionTypeStoryTemplate] != 4 {
		t.Fatalf("story template asset count = %d, want 4", counts[core.CoupleAlbumOptionTypeStoryTemplate])
	}
	if counts[core.CoupleAlbumOptionTypeStyle] != 4 {
		t.Fatalf("style asset count = %d, want 4", counts[core.CoupleAlbumOptionTypeStyle])
	}
}
...
func TestBuildOptionAssetPlanUsesStableObjectKeysAndPublicURLs(t *testing.T) {
	...
	assertPlannedAsset(t, reports, core.CoupleAlbumOptionTypeLocation, "大理",
		...
		"image_url")
	assertPlannedAsset(t, reports, core.CoupleAlbumOptionTypeStoryTemplate, "first_trip",
		...
		"icon_url")
	assertPlannedAsset(t, reports, core.CoupleAlbumOptionTypeStyle, "film",
...
type fakeOptionRepository struct {
	options     map[string]core.CoupleAlbumOption
	updateCalls int
}

func newFakeOptionRepository(specs []optionAssetSpec) *fakeOptionRepository {
	repository := &fakeOptionRepository{options: map[string]core.CoupleAlbumOption{}}
	for _, spec := range specs {
		option := core.CoupleAlbumOption{
			Type:  spec.Type,
			Value: spec.Value,
		}
...
func (r *fakeOptionRepository) FindOption(optionType, value string) (core.CoupleAlbumOption, error) {
